...
class ScaleFRAILAnswerAPIController(BaseAPIController):
    
    def doc_register_frail_answers(self):
        """
        Documentación Swagger para el método register_frail_answers
        
        Returns:
            dict: Documentación Swagger para el endpoint de registrar respuestas de evaluación de FRAIL
        """
        return {
            "tags": ["Evaluación Geriatrica - FRAIL"],
            "summary": "Registrar respuestas de evaluación de FRAIL para un residente",
            "description": """
            Endpoint para registrar una evaluación completa de FRAIL de un residente,
            ...
            **Proceso:**
            ...
            4. Crea una evaluación de FRAIL
                ...
                },
                {
                    ...
                    "schema": {
                        ...
                        "properties": {
                            "resident_id": {
                                "type": "integer",
                                "description": "Identificador del residente al cual se le aplica la evaluación de FRAIL",
                                "example": 4
                            },
                            "date": {
                                "type": "string",
                            ...
                            "answers": {
                                "type": "array",
                                "description": "Lista de respuestas a las preguntas del cuestionario de FRAIL",
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "question_id": {
                                            "type": "integer",
                                            "description": "Identificador de la pregunta del cuestionario de FRAIL",
                                            "example": 1
                                        },
                                        "answer": {
                                            "type": "integer",
            ...
            "responses": {
                "200": {
                    "description": "Evaluación de FRAIL registrada exitosamente",
                    "headers": {
                        "Content-Type": {
                            "type": "string",
                                ...
                                "properties": {
                                    ...
                                    "data": {
                                        ...
                                        "properties": {
                                            "id": {
                                                "type": "integer",
                                                "description": "Identificador de la evaluación de FRAIL creada",
                                                "example": 6
                                            },
                                            "date": {
                                                "type": "string",
                ...
                "404": {
                    "description": "Residente no encontrado o pregunta de FRAIL no existe",
                    "schema": {
                        "type": "object",
                        "properties": {
                            "status": {
                            ...
                            "message": {
                                "type": "string",
                                "example": "Residente no encontrado o pregunta de FRAIL no existe en el sistema"
                            },
                            "data": {
                                "type": "null",
                                "example": None
    ...
    def register_frail_answers(self, **kwargs):
        try:
            parameters = [
                'resident_id',
                'answers',
            ...
            with registry.cursor() as cr:
                ...
                ScaleFRAILAnswer = env['scalefrail.answer'].sudo()
                ScaleFRAILQuestion = env['scalefrail.question'].sudo()
                ScaleFRAILAssessment = env['scalefrail.assessment'].sudo()
                ...
                if not self._check_user_permissions(user, 'scalefrail.answer', self.CAN_CREATE, env):
                    raise AccessDenied("El usuario no tiene los permisos para esta operación")
                if not self._check_user_permissions(user, 'scalefrail.assessment', self.CAN_CREATE, env):
                    raise AccessDenied("El usuario no tiene los permisos para esta operación")
 ...
 usuario se autentico")
                ...
                for index, obj in enumerate(frail_answers):
                    ...
                    question = ScaleFRAILQuestion.browse(question_id)
                    
                    if not question:
                        raise Exception("No existe una pregunta de FRAIL en el sistema con ese identificador")
                    if not question.active:
                        raise Exception("La pregunta de estado de FRAIL no se encuentra activa ")

                date_adjust = self._adjust_timezone(user, data['date'])
                # - Crear evaluacion
                assessment = ScaleFRAILAssessment.create({

...
class ScaleSARCFAnswerAPIController(BaseAPIController):

    def doc_register_sarcf_answers(self):
        """
        Documentación Swagger para el método register_sarcf_answers

        Returns:
            dict: Documentación Swagger para el endpoint de registrar respuestas de evaluación de SARC-F
        """
        return {
            "tags": ["Evaluación Geriatrica - SARC-F"],
            "summary": "Registrar respuestas de evaluación de SARC-F para un residente",
            "description": """
            Endpoint para registrar una evaluación completa de SARC-F de un residente,
            ...
            **Proceso:**
            ...
            4. Crea una evaluación de SARC-F
                ...
                },
                {
                    ...
                    "schema": {
                        ...
                        "properties": {
                            "resident_id": {
                                "type": "integer",
                                "description": "Identificador del residente al cual se le aplica la evaluación de SARC-F",
                                "example": 4
                            },
                            "observations": {
                                "type": "string",
                                "description": "Cadena de texto o vacía que contiene alguna observación reerente al residente cuando se le aplica la evaluación de SARC-F",
                            ...
                            "answers": {
                                "type": "array",
                                "description": "Lista de respuestas a las preguntas del cuestionario de SARC-F",
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "question_id": {
                                            "type": "integer",
                                            "description": "Identificador de la pregunta del cuestionario de SARC-F",
                                            "example": 1
                                        },
                                        "choise_id": {
                                            "type": "integer",
            ...
            "responses": {
                "200": {
                    "description": "Evaluación de SARC-F registrada exitosamente",
                    "headers": {
                        "Content-Type": {
                            "type": "string",
                                ...
                                "properties": {
                                    ...
                                    "data": {
                                        ...
                                        "properties": {
                                            "id": {
                                                "type": "integer",
                                                "description": "Identificador de la evaluación de SARC-F creada",
                                                "example": 6
                                            },
                                            "date": {
                                                "type": "string",
                ...
                "404": {
                    "description": "Residente no encontrado o pregunta de SARC-F no existe",
                    "schema": {
                        "type": "object",
                        "properties": {
                            "status": {
                            ...
                            "message": {
                                "type": "string",
                                "example": "Residente no encontrado o pregunta de SARC-F no existe en el sistema"
                            },
                            "data": {
                                "type": "null",
                                "example": None
    ...
    def register_sarcf_answers(self, **kwargs):
        try:
            parameters = [
                'resident_id',
                'answers',
            ...
            with registry.cursor() as cr:
                ...
                ScaleSARCFAnswer = env['scalesarcf.answer'].sudo()
                ScaleSARCFQuestion = env['scalesarcf.question'].sudo()
                ScaleSARCFChoise = env['scalesarcf.choise'].sudo()
                ScaleSARCFAssessment = env['scalesarcf.assessment'].sudo()
                ...
                if not self._check_user_permissions(user, 'scalesarcf.answer', self.CAN_CREATE, env):
                    raise AccessDenied("El usuario no tiene los permisos para esta operación")
                if not self._check_user_permissions(user, 'scalesarcf.assessment', self.CAN_CREATE, env):
                    raise AccessDenied("El usuario no tiene los permisos para esta operación")
 ...
 usuario se autentico")
                ...
                for index, obj in enumerate(sarcf_answers):
                    ...
                    question = ScaleSARCFQuestion.browse(question_id)
                    choise = ScaleSARCFChoise.browse(choise_id)

                    if not question:
                        raise Exception("No existe una pregunta de SARC-F en el sistema con ese identificador")
                    if not question.active:
                        raise Exception("La pregunta de estado de SARC-F no se encuentra activa ")
                    if not choise:
                        raise Exception("No existe una opción selecionada como respuesta de una pregunta SARC-F en el sistema con ese identificador")
                    if not question.is_option_response(choise_id):
                        raise Exception("La opción seleccionada no es parte de las opciones de respuesta para la pregunta")
                
                date_adjust = self._adjust_timezone(user, data['date'])
                # - Crear evaluacion
                assessment = ScaleSARCFAssessment.create({

...
class MedicamentSubstance(models.Model):
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'medicament.substance', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'medicament.substance', 'write', extra_details=details)
        return result
    ...
    def unlink(self):
        ...
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'medicament.substance', 'unlink')
        return super().unlink()

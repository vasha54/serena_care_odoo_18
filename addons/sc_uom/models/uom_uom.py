...
class UoMUoM(models.Model):
    ...
    def create(self, values):
        # Lógica original para marcar como UoM de Serena Care si viene en el contexto
        if 'uom_sc' in self.env.context:
            values['is_uom_sc'] = True
        records = super().create(values)
        # Crear log de auditoría para cada registro creado
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'uom.uom', 'create')
        return records
    ...
    def write(self, values):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'uom.uom', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'uom.uom', 'unlink')
        return super().unlink()

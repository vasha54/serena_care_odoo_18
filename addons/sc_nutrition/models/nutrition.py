...
class Nutrition(models.Model):
    ...
    nutrition_level_id = fields.Many2one(
        'nutrition.level', 
        string="Nivel de alimentación", 
        required=True,
        domain=[('active','=',True)]
    )
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = f"Alimentación de {r.resident_id.name}"
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'nutrition', 'create')
        return records
            
    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'nutrition', 'unlink')
        return super().unlink()
    ...
    def write(self, values):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'nutrition', 'write', extra_details=details)
        return result

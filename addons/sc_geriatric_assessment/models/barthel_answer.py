...
class BarthelAnswer(models.Model):
    _name = "barthel.answer"
    ...
    assessment_id = fields.Many2one(
        "barthel.assessment", string="Evaluación", required=True
    )
    choise_select_id = fields.Many2one(
        "barthel.choise", string="Opción seleccionada", required=True
    )
    ...
    question_id = fields.Many2one("barthel.question", string="Actividad", required=True)
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = f"Respuesta dada a la pregunta: {r.question_id.name} realizada {r.create_date.strftime('%d/%m/%Y %H:%M')}"
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.answer', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.answer', 'write', extra_details=details)
        return result
    
    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.answer', 'unlink')
        return super().unlink()
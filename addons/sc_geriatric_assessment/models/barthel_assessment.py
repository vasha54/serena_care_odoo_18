...
class BarthelAssessment(models.Model):
    _name = "barthel.assessment"
    _description = "Evaluación del Barthel del residente"
    _order = 'date desc'
    ...
    question_answers = fields.One2many(
        "barthel.answer", "assessment_id", string="Respuestas"
    )
    ...
    barthel_state_id = fields.Many2one(
        "barthel.state",
        string="Estado de ánimo",
        compute="_compute_barthel_state",
        ondelete="restrict",
        store=True,
    )
    ...
    barthel_state_display = fields.Char(
        string="Resultado",
        compute="_compute_barthel_state_display",
        index=True,
        store=True,
    )
    ...
    def _compute_barthel_state(self):
        BarthelState = self.env["barthel.state"].sudo()
        for asses in self:
            state = BarthelState.search(
                [
            ...
            asses.barthel_state_id = state[0] if state else None

    @api.depends("barthel_state_id")
    def _compute_barthel_state_display(self):
        for asses in self:
            asses.barthel_state_display = (
                asses.barthel_state_id.name
                if asses.barthel_state_id
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = f"Evaluación de Barthel de {r.resident_id.name} realizada {r.create_date.strftime('%Y-%m-%d %H:%M')}"
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.assessment', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.assessment', 'write', extra_details=details)
        return result
    
    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.assessment', 'unlink')
        return super().unlink()

...
class LawtonBrodyQuestion(models.Model):
    _name = 'lawtonbrody.question'
    _description = 'Pregunta para Lawton-Brody'

    active = fields.Boolean(string='Activo', default=True)
    name = fields.Char(string='Título', required=True)
    order = fields.Integer(string="Orden de secuencia", default=1)
    choise_ids = fields.Many2many('lawtonbrody.choise',
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.question', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.question', 'write', extra_details=details)
        return result
    
    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.question', 'unlink')
        return super().unlink()

...
class ScaleSARCFChoise(models.Model):
    _name = 'scalesarcf.choise'
    _description = 'Opción de respuesta a una pregunta de SARC-F'

    name = fields.Char(string='Opción', required=True)
    value = fields.Float(string='Valor', required=True)
    question_ids = fields.Many2many('scalesarcf.question', string='Preguntas')

    @api.constrains('name', 'valor')
    ...
    def create(self, vals):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.choise', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.choise', 'write', extra_details=details)
        return result
    ...
    def unlink(self):
        for record in self:
            ...
            answer_count = self.env['scalesarcf.answer'].search_count([
        ...
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.choise', 'unlink')
        return super().unlink()

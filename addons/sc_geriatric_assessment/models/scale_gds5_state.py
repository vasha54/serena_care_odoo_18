...
class ScaleGDS5State(models.Model):
    _name = 'scalegds5.state'
    _description = 'Estado de GDS-5'
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalegds5.state', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalegds5.state', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalegds5.state', 'unlink')
        return super().unlink()

...
class LawtonBrodyState(models.Model):
    _name = 'lawtonbrody.state'
    _description = 'Estado de Lawton-Brody'
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.state', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.state', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'lawtonbrody.state', 'unlink')
        return super().unlink()

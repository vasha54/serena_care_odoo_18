...
class BarthelChoise(models.Model):
    _name = 'barthel.choise'
    _description = 'Opción de respuesta a una pregunta de Barthel'

    name = fields.Char(string='Opción', required=True)
    value = fields.Float(string='Valor', required=True)
    question_ids = fields.Many2many('barthel.question', string='Preguntas')
    ...
    def create(self, vals):
        ...
        records = super(BarthelChoise, self).create(vals)
        # Crear log de auditoría para cada registro creado
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.choise', 'create')
        return records
    ...
    def write(self, vals):
        ...
        result = super(BarthelChoise, self).write(vals)
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.choise', 'write', extra_details=details)
        return result
    ...
    def unlink(self):
        for record in self:
            ...
            answer_count = self.env['barthel.answer'].search_count([
                ('choise_select_id', '=', record.id)
            ])
            if answer_count > 0:
                raise ValidationError("No se puede eliminar una opción que está siendo utilizada en respuestas.")
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'barthel.choise', 'unlink')
        
        return super(BarthelChoise, self).unlink()

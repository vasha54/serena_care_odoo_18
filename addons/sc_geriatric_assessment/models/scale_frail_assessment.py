...
class ScaleFRAILAssessment(models.Model):
    _name = "scalefrail.assessment"
    _description = "Evaluación de la escala de FRIAL del residente"
    _order = 'date desc'
    ...
    question_answers = fields.One2many(
        "scalefrail.answer", "assessment_id", string="Respuestas"
    )
    ...
    scalefrail_state_id = fields.Many2one(
        "scalefrail.state",
        string="Estado de fragilidad",
        compute="_compute_fragile_state",
        ondelete="restrict",
        store=True,
    )
    ...
    scalefrail_state_display = fields.Char(
        string="Estado de fragilidad",
        compute="_compute_fragile_state_display",
        index=True,
        store=True,
    )
    ...
    def _compute_total_score(self):
        for asses in self:
            asses.total_score = sum(
                1 for answer in asses.question_answers if answer.answer_select == "1"
            )

    @api.depends("total_score", "question_answers")
    def _compute_fragile_state(self):
        ScaleFrailState = self.env["scalefrail.state"].sudo()
        for asses in self:
            state = ScaleFrailState.search(
                [
            ...
            asses.scalefrail_state_id = state[0] if state else None

    @api.depends("scalefrail_state_id")
    def _compute_fragile_state_display(self):
        for asses in self:
            asses.scalefrail_state_display = (
                asses.scalefrail_state_id.name
                if asses.scalefrail_state_id
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = f"Evaluación de FRAIL de {r.resident_id.name} realizada {r.create_date.strftime('%Y-%m-%d %H:%M')}"
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.assessment', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.assessment', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.assessment', 'unlink')
        return super().unlink()
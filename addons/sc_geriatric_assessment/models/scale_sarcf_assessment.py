...
class ScaleSARCFAssessment(models.Model):
    _name = "scalesarcf.assessment"
    _description = "Evaluación del SARC-F del residente"
    _order = 'date desc'
    ...
    question_answers = fields.One2many(
        "scalesarcf.answer", "assessment_id", string="Respuestas"
    )
    ...
    scalesarcf_state_id = fields.Many2one(
        "scalesarcf.state",
        string="Estado de ánimo",
        compute="_compute_scalesarcf_state",
        ondelete="restrict",
        store=True,
    )
    ...
    scalesarcf_state_display = fields.Char(
        string="Resultado",
        compute="_compute_scalesarcf_state_display",
        index=True,
        store=True,
    )
    ...
    def _compute_scalesarcf_state(self):
        ScaleSARCFState = self.env["scalesarcf.state"].sudo()
        for asses in self:
            state = ScaleSARCFState.search(
                [
            ...
            asses.scalesarcf_state_id = state[0] if state else None

    @api.depends("scalesarcf_state_id")
    def _compute_scalesarcf_state_display(self):
        for asses in self:
            asses.scalesarcf_state_display = (
                asses.scalesarcf_state_id.name
                if asses.scalesarcf_state_id
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = f"Evaluación de SARC-F de {r.resident_id.name} realizada {r.create_date.strftime('%Y-%m-%d %H:%M')}"
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.assessment', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.assessment', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalesarcf.assessment', 'unlink')
        return super().unlink()
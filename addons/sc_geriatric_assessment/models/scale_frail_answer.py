...
class ScaleFRAILAnswer(models.Model):
    _name = "scalefrail.answer"
    ...
    assessment_id = fields.Many2one(
        "scalefrail.assessment", string="Evaluación", required=True
    )
    question_id = fields.Many2one(
        "scalefrail.question", string="Pregunta", required=True
    )
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.answer', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.answer', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'scalefrail.answer', 'unlink')
        return super().unlink()
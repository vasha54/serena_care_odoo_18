...
class WaterBalanceAnnotation(models.Model):
    _inherit = 'water.balance.annotation'
    ...
    type_annotation_label = fields.Char(
        string='Nivel de Riesgo',
        compute='_compute_type_annotation_label',
        store=False
    )
    
    @api.depends('type_annotation')
    def _compute_type_annotation_label(self):
        for record in self:
            record.type_annotation_label = dict(
                self._fields['type_annotation'].selection
            ).get(record.type_annotation, '')

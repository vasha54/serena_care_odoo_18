...
class EvacuationType(models.Model):
    _name = 'evacuation.type'
    _description = 'Tipos de Evacuación'
        ...
        for vals in values:
            ...
            if existing_record_slug:
                raise ValidationError(
                    f"Un tipo de evacuación con nombre '{vals['name']}' ya "
                    f"existe."
                )
        records = super().create(values)
        # Crear log de auditoría para cada registro creado
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'evacuation.type', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'evacuation.type', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'evacuation.type', 'unlink')
        return super().unlink()

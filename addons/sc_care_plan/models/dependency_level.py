...
class DependencyLevel(models.Model):
    _name = 'dependency.level'
    _description = 'Nivel de Dependencia'
        ...
        for vals in values:
            ...
            if existing_record_slug:
                raise ValidationError(
                    f"Un nivel de dependencia con nombre '{vals['name']}' ya existe."
                )
        records = super().create(values)
        # Crear log de auditoría para cada registro creado
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'dependency.level', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'dependency.level', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'dependency.level', 'unlink')
        return super().unlink()

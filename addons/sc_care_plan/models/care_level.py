...
class DependencyLevel(models.Model):
    ...
    def create(self, values):
        ...
        for record in records:
            self.env['audit.log'].sudo().crud_audit_log(record, 'care.level', 'create')
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            self.env['audit.log'].sudo().crud_audit_log(record, 'care.level', 'write', extra_details=details)
        return result

    def unlink(self):
        # Antes de eliminar, crear logs para cada registro
        for record in self:
            self.env['audit.log'].sudo().crud_audit_log(record, 'care.level', 'unlink')
        return super().unlink()
